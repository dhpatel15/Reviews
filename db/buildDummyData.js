...
const userDataMaker = () => {
  if (Math.ceil(Math.random() * 5) > 1) {
    var first = firstNames[Math.ceil(Math.random() * firstNames.length)];
    if (Math.ceil(Math.random() * 3) > 1) {
      var last = lastNames[Math.ceil(Math.random() * lastNames.length)];
    } else {
      var last = '';
    }
  } else {
